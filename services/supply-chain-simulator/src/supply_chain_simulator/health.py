...
The simulator is a synchronous worker, so rather than pull in an async web framework it serves
``/health/live`` and ``/health/ready`` from a stdlib threaded HTTP server. Readiness pings
Postgres. Prometheus ``/metrics`` is served separately by ``prometheus_client`` on ``METRICS_PORT``.
"""
...
def _make_handler(is_ready: ReadinessProbe) -> type[BaseHTTPRequestHandler]:
    class _Handler(BaseHTTPRequestHandler):
        ...
        def log_message(self, *_args: object) -> None:
            # Silence default stderr access logging; structured logs come from the app.
            return
